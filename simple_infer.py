...
Usage:
    python simple_infer.py \
        --model_path output_dir/rl_mind_cot/final_checkpoint \
...
# Reuse the same prompt builder from eval to guarantee alignment
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), "src"))
from evaluate_mind_ranking import (
    build_cot_prompt_content,
...
def main():
    ...
    news_data = load_news(args.news_path, use_abstract=False)
    ...
    for idx, (impression_id, history_objs, candidate_objs, labels) in enumerate(samples):
        ...
        # Extract answer
        answer = extract_cot_answer(generated_text, len(candidate_objs))

        print(generated_text)
        print(f"\n--- RESULT ---")
        print(f"Extracted answer: {answer}")
        print(f"Correct: {answer in gt_positives if answer else False}")
        print(f"Generated {n_tokens} tokens in {gen_time:.2f}s ({n_tokens/gen_time:.1f} tok/s)")

        if answer and answer in gt_positives:
            correct += 1
